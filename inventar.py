...
def ausleihe_erstellen(daten):
    """Erstellt eine Ausleihe und reduziert die verfügbare Menge."""
...
def ausleihe_zuruecknehmen(ausleihe_id, rueckgabe_daten, menge_defekt_bei_rueckgabe=0):
    """Nimmt eine Ausleihe zurück und verbucht Schäden direkt als defekt im Lager."""
    ...
    try:
        ...
        if inventar_id:
            ...
            if inv_res.data:
                ...
                neuer_status = "Verfügbar" if neue_verfuegbar > 0 else ("Defekt" if neue_defekt >= gesamt else "Ausgeliehen")
                
                supabase.table("inventar").update({
                    "menge_verfuegbar": neue_verfuegbar,
                    "menge_defekt": neue_defekt,
                    "status": neuer_status
                }).eq("id", inventar_id).execute()
...
def material_reparieren(inventar_id, anzahl_repariert):
    """Schraubenschlüssel-Funktion: Bucht defektes Material wieder ins verfügbare Lager zurück."""
    try:
        inv_res = supabase.table("inventar").select("menge_gesamt, menge_verfuegbar, menge_defekt").eq("id", inventar_id).single().execute()
        if not inv_res.data:
        ...
        akt_repariert = min(defekt, anzahl_repariert)
        neue_defekt = defekt - akt_repariert
        neue_verfuegbar = min(gesamt, verfuegbar + akt_repariert)
        
        neuer_status = "Verfügbar" if neue_verfuegbar > 0 else "Gesperrt"
        
        return supabase.table("inventar").update({
            "menge_verfuegbar": neue_verfuegbar,
            "menge_defekt": neue_defekt,
            "status": neuer_status
        }).eq("id", inventar_id).execute()
