...
def show():
    ...
    # Tabs dynamisch je nach Rechten aufbauen
    if has_inventar_rights or is_admin_or_vorstand:
        tab_uebersicht, tab_ausleihe, tab_verwaltung, tab_einstellungen = st.tabs([
            "📋 Inventar-Liste",
            "🔄 Ausleihe & Rückgabe",
            "⚙️ Inventar pflegen",
            "🔔 Einstellungen"
        ...
        tab_uebersicht, tab_ausleihe = st.tabs([
            "📋 Inventar-Liste",
            "🔄 Ausleihe & Rückgabe"
        ])
        tab_verwaltung = None
    ...
    with tab_ausleihe:
        ...
        with sub_tab1:
            ...
            if verfuegbare_items:
                ...
                with st.form("ausleihe_form"):
                    ...
                    with col1:
                        person_name = st.text_input("Name der ausleihenden Person *")
                    ...
                    if submitted:
                        if not person_name:
                            st.error("Bitte gib den Namen der ausleihenden Person an.")
                        else:
                            ...
                            try:
                                ...
                                st.success(f"Ausleihe für '{selected_item.get('name')}' erfolgreich eingetragen!")
                                st.rerun()
        ...
        with sub_tab2:
            st.subheader("Aktive Ausleihen verwalten")
            ...
            if aktive_ausleihen:
                ...
                with st.form("rueckgabe_form"):
                    ...
                    with col1:
                        ...
                        rueckgeber_name = st.text_input("Zurückgebracht von", value=selected_ausleihe.get('person_name'))
                    with col2:
                        festgestellte_maengel = st.text_input("Festgestellte Mängel (falls vorhanden)")
                        schadensbericht_neu = st.text_area("Schadensbericht / Notizen zur Rücknahme")
                    ...
                    if ruecknahme_btn:
                        ...
                        try:
                            ausleihe_zuruecknehmen(selected_ausleihe.get("id"), rueckgabe_daten)
                            st.success("Gegenstand erfolgreich zurückgenommen und Bestand aktualisiert!")
                            st.rerun()
                        except Exception as e:
    ...
    # ==========================================
    # 3. INVENTAR PFLEGEN (Nur mit Rechten)
    # ==========================================
    if (has_inventar_rights or is_admin_or_vorstand) and tab_verwaltung is not None:
        with tab_verwaltung:
    ...
    # 4. EINSTELLUNGEN
    # ==========================================
    if (has_inventar_rights or is_admin_or_vorstand) and tab_einstellungen is not None:
